...
Report bugs at https://github.com/MeteoSwiss-APN/fls_sat_verif/issues.
...
fls_sat_verif could always use more documentation, whether as part of the official fls_sat_verif docs, in docstrings, or even on the web in blog posts, articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/MeteoSwiss-APN/fls_sat_verif/issues.
...
Ready to contribute? Here's how to set up `fls_sat_verif` for local development.

1. Fork the `fls_sat_verif` repo on GitHub.
2. Clone your fork locally::

        $ git clone git@github.com:your_name_here/fls_sat_verif.git
...
3. Create a virtual environment and install the development dependencies::
    ...
    This will create a virtual environment named ``fls_sat_verif`` (change with ``VENV_NAME=my_venv``) in ``./venv`` (change with ``VEND_DIR=path/to/venv``) and install the following dependencies:
...
To run a subset of tests::
    ...
    $ pytest tests.test_fls_sat_verif
...
.. list-table:: Structure
   ...
   * -  src/
     -  Source folder, with the main package in ``src/fls_sat_verif``.
   * -  tests/
   ...
   * -  USAGE.txt
     -  Instruction on using fls_sat_verif
   * -  VERSION
...
By default, a single executable script called fls_sat_verif is provided.
It is created when the package is installed.
When you call it, the main function (``cli``) in ``src/fls_sat_verif/cli.py`` is called.
...
When the package is installed, a executable script is created in the Python's bin folder with the name ``fls_sat_verif``.
If a user calls this script, the function ``main`` in the file ``src/fls_sat_verif/cli.py`` is called.
